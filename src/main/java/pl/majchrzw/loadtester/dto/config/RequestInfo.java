...
import pl.majchrzw.loadtester.shared.HeadersValidator;
...
public record RequestInfo(
	...
	public ConfigValidationStatus validate() {
		...
		if (!HeadersValidator.validateHeaders(headers).valid())
			return new ConfigValidationStatus(false, MessageFormat.format("Headers in request {0} are not valid, see RFC 2616 for proper header definition", name));
